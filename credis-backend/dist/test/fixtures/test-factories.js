...
export class TestFactories {
    ...
    static createMultipleCustomers(count) {
        return Array.from({ length: count }, (_, index) => this.createCustomer({
            id: `cust_${index + 1}`,
            email: `customer${index + 1}@example.com`
        }));
    }
    static createMultipleStores(count) {
        return Array.from({ length: count }, (_, index) => this.createStore({
            id: `store_${index + 1}`,
            name: `Store ${index + 1}`,
