...
customersRouter.post('/', async (c) => {
    try {
        ...
        const { storeId, name, phoneNumber, address, email, cidNumber, creditLimit } = body;
        // Validate required fields
        if (!storeId || !name || !phoneNumber) {
            return c.json({ error: 'Store ID, name, and phone number are required' }, 400);
        }
        ...
        if (existingCustomer) {
            return c.json({ error: 'Customer with this phone number already exists in this store' }, 409);
        }
        // Create customer
