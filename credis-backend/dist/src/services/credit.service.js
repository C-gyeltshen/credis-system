...
export class CreditService {
    ...
    async createCredit(data) {
        ...
        if (data.transaction_type === "credit_given" && customer.creditLimit) {
            const currentBalance = await this.repository.getCustomerSummary(data.customer_id, data.store_id);
            const newBalance = currentBalance.outstanding_balance + data.amount;
            if (newBalance > Number(customer.creditLimit)) {
                throw new Error(`Credit limit exceeded. Current balance: ${currentBalance.outstanding_balance}, Credit limit: ${customer.creditLimit}`);
            }
        }
        ...
        let lastCreditDate = undefined;
        let lastPaymentDate = undefined;
        let lastTransactionDate = undefined;
    ...
    async getCreditsByFilters(filters) {
        // Validate customer if provided
        if (filters.customer_id) {
            const customer = await this.customerRepository.findById(filters.customer_id);
            if (!customer) {
                throw new Error("Customer not found");
            }
        }
    ...
    async updateCredit(id, data) {
        ...
        if ((data.amount !== undefined || data.transaction_type !== undefined) &&
            existingCredit.customer) {
            const newTransactionType = data.transaction_type || existingCredit.transactionType;
            ...
            if (newTransactionType === "credit_given" &&
                ...
                const currentSummary = await this.repository.getCustomerSummary(existingCredit.customerId, existingCredit.storeId);
                ...
                if (adjustedBalance > Number(existingCredit.customer.creditLimit)) {
                    throw new Error(`Updated transaction would exceed credit limit. Credit limit: ${existingCredit.customer.creditLimit}`);
                }
            }
        }
    ...
    async getTransactionsByDateRange(startDate, endDate, storeId) {
        ...
        return await this.repository.getTransactionsByDateRange(startDate, endDate, storeId);
    }
    async getCustomersWithOutstandingBalance(storeId, limit) {
        // Validate store exists
        const store = await this.storeRepository.findById(storeId);
        if (!store) {
            throw new Error("Store not found");
        ...
        for (const transaction of transactions) {
            ...
            const creditGiven = transaction.transactionType === "credit_given"
                ? Number(transaction.amount)
                : 0;
            const paymentsReceived = transaction.transactionType === "payment_received"
                ? Number(transaction.amount)
                : 0;
            if (existing) {
